...
const char * const shitecoin_CONF_FILENAME = "shitecoin.conf";
const char * const shitecoin_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/shitecoin/shitecoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //shitecoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", shitecoin_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "shitecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\shitecoin
    // macOS: ~/Library/Application Support/shitecoin
    // Unix-like: ~/.shitecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "shitecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/shitecoin";
#else
    // Unix-like
    return pathRet / ".shitecoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", shitecoin_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure shitecoin Core copyright is not removed by accident
    if (copyright_devs.find("shitecoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The shitecoin Core developers";
    }
