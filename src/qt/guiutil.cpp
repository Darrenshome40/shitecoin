...
#include <qt/shitecoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a shitecoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new shitecoinAddressEntryValidator(parent));
    widget->setCheckValidator(new shitecoinAddressCheckValidator(parent));
}

bool parseshitecoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no shitecoin: URI
    if(!uri.isValid() || uri.scheme() != QString("shitecoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!shitecoinUnits::parse(shitecoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseshitecoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseshitecoinURI(uriInstance, out);
}

QString formatshitecoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("shitecoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(shitecoinUnits::format(shitecoinUnits::BTC, info.amount, false, shitecoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openshitecoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", shitecoin_CONF_FILENAME));
    ...
    /* Open shitecoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "shitecoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "shitecoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("shitecoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for shitecoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "shitecoin.desktop";
    return GetAutostartDir() / strprintf("shitecoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a shitecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=shitecoin\n";
        else
            optionFile << strprintf("Name=shitecoin (%s)\n", chain);
