...
shitecoin Core version 0.20.0 is now available from:

  <https://shitecoincore.org/bin/shitecoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  shitecoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use shitecoin Core on
unsupported systems.

From shitecoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
...
- Instead of running `make` simply, you should instead run
  `shitecoin_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from shitecoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since shitecoin Core version 0.18.0.
...
- Testing or debugging of implementations of the shitecoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of shitecoin Core. shitecoin Core logs debug messages
...
- OpenSSL is no longer used by shitecoin Core.  (#17265)

- BIP70 support has been fully removed from shitecoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/shitecoin/shitecoin/issues/16599),
  [PR #16702](https://github.com/shitecoin/shitecoin/pull/16702), and the
  `shitecoind help` for more information.  This option is experimental and
...
- All custom settings configured when shitecoin Core starts are now
...
- The "Start shitecoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://shitecoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple shitecoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed shitecoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/shitecoin-core/HWI) for
  signing.  Future versions of shitecoin Core should support a GUI option
...
- shitecoin Core's automatically-generated source code documentation is
  now available at https://doxygen.shitecoincore.org.  (#17596)
...
- The `shitecoin-cli` utility used with the `-getinfo` parameter now
...
- The `shitecoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `shitecoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_shitecoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBshitecoin_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_shitecoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBshitecoin_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for shitecoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate shitecoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest shitecoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_shitecoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (shitecoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone shitecoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about shitecoin Core files (hebasto)
...
- #17119 Fix broken shitecoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from shitecoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.shitecoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to shitecoin-wallet (hel-o)
...
- #16115 On shitecoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from shitecoin-cli -getinfo (malevolent)
