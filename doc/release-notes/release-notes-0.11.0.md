shitecoin Core version 0.11.0 is now available from:

  <https://shitecoin.org/bin/shitecoin-core-0.11.0/>
...
For example, add the following to `shitecoin.conf`:
...
related to the blockchain in the shitecoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows shitecoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that shitecoin Core will 
...
For an example script see [shitecoin-submittx](https://github.com/laanwj/shitecoin-submittx).
...
- #5334 `cf87536` libshitecoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new shitecoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only shitecoinAmountField
- #5753 `0f44672` Add shitecoin logo to about screen
...
- #5809 `46bfbe7` Add shitecoin-cli man page
...
- #5849 `41259ca` contrib/init/shitecoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename shitecoin-Qt.app to shitecoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for shitecoin-tx)
