shitecoin Core version 0.18.0 is now available from:

  <https://shitecoincore.org/bin/shitecoin-core-0.18.0/>
...
`/Applications/shitecoin-Qt` (on Mac) or `shitecoind`/`shitecoin-qt` (on
Linux).
...
recommended to use shitecoin Core on unsupported systems.

shitecoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, shitecoin Core does not yet change appearance when
...
`contrib/init/shitecoind.service`, it has been changed to use
`/var/lib/shitecoind` as the data directory instead of
`~shitecoin/.shitecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/shitecoind` will
exist has enough space (check using `df -h /var/lib/shitecoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/shitecoin/shitecoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://shitecoincore.org/en/contact/) the shitecoin Core
...
The systemd init file (`contrib/init/shitecoind.service`) has been changed
to use `/var/lib/shitecoind` as the data directory instead of
`~shitecoin/.shitecoin`. This change makes shitecoin Core more consistent with
...
`datadir` option in `/etc/shitecoin/shitecoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/shitecoin/shitecoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/shitecoin/shitecoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/shitecoin/shitecoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/shitecoin/shitecoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/shitecoin/shitecoin/blob/master/doc/shitecoin-conf.md)
  about the `shitecoin.conf` file describes how to use it to configure
  shitecoin Core.

- A new document introduces shitecoin Core's BIP174 [Partially-Signed
  shitecoin Transactions
  (PSBT)](https://github.com/shitecoin/shitecoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/shitecoin/shitecoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent shitecoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  shitecoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/shitecoin/shitecoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/shitecoin/shitecoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when shitecoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of shitecoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/shitecoin/shitecoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/shitecoin/shitecoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart shitecoind with the `-deprecatedrpc=generate`
...
- A new `shitecoin-wallet` tool is now distributed alongside shitecoin
...
This section describes planned changes to shitecoin Core that may affect
other shitecoin software and services.

- Since version 0.16.0, shitecoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with shitecoin Core 0.20 (expected about
  a year after 0.18), shitecoin Core will default to native segwit
  ...
  to bech32 addresses, and if the shitecoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in shitecoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any shitecoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, shitecoin Core
...
- A sub-project of shitecoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with shitecoin Core.  See their [project
  page](https://github.com/shitecoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to shitecoin Core's own implementation, although entropy
  gathered by shitecoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves shitecoin Core a
...
- On macOS, shitecoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `shitecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libshitecoin_server.a` (jnewbery)
...
- #13926 [Tools] shitecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like shitecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `shitecoin-qt -version` output formatting (hebasto)
...
- #11625 Add shitecoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libshitecoin_server` (laanwj)
...
- #15154 Configure: shitecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/shitecoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for shitecoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libshitecoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in shitecoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into shitecoin Core (Christewart)
...
- #12246 Bugfix: Only run shitecoin-tx tests when shitecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_shitecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_shitecoin` (MarcoFalke)
...
- #14186 shitecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_shitecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update shitecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/shitecoin-conf.md (hebasto)
...
- #15354 Add missing `shitecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct shitecoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support shitecoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/shitecoin/).
