...
The shitecoin Core daemon can be configured to act as a trusted "border
router", implementing the shitecoin wire protocol and relay, making
...
The ZeroMQ feature in shitecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building shitecoind:
...
For instance:
    ...
    $ shitecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/shitecoind.tx.raw \
...
These options can also be provided in shitecoin.conf.
...
From the perspective of shitecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into shitecoind directly. Furthermore, no information is
...
using. shitecoind appends an up-counting sequence number to each
